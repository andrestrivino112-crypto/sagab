...
| Cuentas demo rotas (`username` NULL) | `database/11_fix_usernames_demo.sql`, aplicado a la BD. `docente@`/`auditor@`/`padre@bellini.edu.ec` ya inician sesión (usuario `carlos.perez`/`ana.auditora`/`luis.morales`, contraseña `***REDACTED***`). |
