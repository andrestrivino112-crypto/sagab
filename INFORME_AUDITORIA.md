...
- **Proveedor local añadido en la Parte 2**: si no se define ninguna variable `SAGAB_S3_*`, la aplicación persiste archivos en `SAGAB_STORAGE_LOCAL_DIR` y emite enlaces HMAC temporales. En producción ese directorio debe ser un volumen persistente. Una configuración S3 parcial se rechaza; si se define el conjunto completo, S3/R2 conserva la prioridad.
